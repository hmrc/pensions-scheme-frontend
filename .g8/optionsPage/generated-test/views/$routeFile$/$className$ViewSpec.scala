    ...
    "rendered" must {
      "contain radio buttons for the value" in {
        ...
        for (option <- $className$.options) {
          assertContainsRadioButton(doc, "value", "value", option.value, false)
        }
      }
    }
    ...
    for(option <- $className$.options) {
      s"rendered with a value of '\${option.value}'" must {
        s"have the '\${option.value}' radio button selected" in {
          ...
          assertContainsRadioButton(doc, "value", "value", option.value, true)

          for(unselectedOption <- $className$.options.filterNot(o => o == option)) {
            assertContainsRadioButton(doc, "value", "value", unselectedOption.value, false)
          }
        }
      }
    }
