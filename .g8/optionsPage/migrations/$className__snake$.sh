...
echo "messages__$className;format="decap"$__option1 = $className;format="decap"$" Option 1 >> ../conf/messages.en
echo "messages__$className;format="decap"$__option2 = $className;format="decap"$" Option 2 >> ../conf/messages.en
...
awk '/class/ {\
     ...
     print "    x => AnswerRow(\"messages__$className;format="decap"$__checkYourAnswersLabel\", Seq(s\"$className;format="decap"$.\$x\"), true, controllers.$routeFile$.routes.$className$Controller.onPageLoad(CheckMode).url)";\
     print "  }";\
