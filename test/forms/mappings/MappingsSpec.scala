...
class MappingsSpec extends WordSpec with MustMatchers with OptionValues with Mappings {
  ...
  "companyDirectorNino" must {

    val testForm: Form[CompanyDirectorNino] = Form("companyDirectorNino" ->  companyDirectorNinoMapping())

    "fail to bind when yes is selected but NINO is not provided" in {
      val result = testForm.bind(Map("companyDirectorNino.hasNino" -> "true"))
      result.errors mustEqual Seq(FormError("companyDirectorNino.nino", "messages__error__nino"))
    }

    "fail to bind when no is selected but reason is not provided" in {
      val result = testForm.bind(Map("companyDirectorNino.hasNino" -> "false"))
      result.errors mustEqual Seq(FormError("companyDirectorNino.reason", "messages__companyDirector__no_nino"))
    }

    Seq("DE999999A", "AO111111B", "ORA12345C", "AB0202020", "AB0303030D", "AB040404E").foreach { nino =>
      s"fail to bind when NINO $nino is invalid" in {
        val result = testForm.bind(Map("companyDirectorNino.hasNino" -> "true", "companyDirectorNino.nino" -> nino))
        result.errors mustEqual Seq(FormError("companyDirectorNino.nino", "messages__error__nino_invalid"))
      }
    }

    "fail to bind when no is selected and reason exceeds max length of 150" in {
      val testString = RandomStringUtils.randomAlphabetic(151)
      val result = testForm.bind(Map("companyDirectorNino.hasNino" -> "false", "companyDirectorNino.reason" -> testString))
      result.errors mustEqual Seq(FormError("companyDirectorNino.reason", "messages__error__no_nino_length", Seq(150)))
    }
  }
