...
package object utils {
  ...
  implicit class UserAnswerOps(answers: UserAnswers) extends OptionValues with Enumerable .Implicits {
    ...
    def establisherCompanyTradingTime(index: Int, tradingTime: AddressYears): UserAnswers = {
      answers.set(establishers.company.CompanyTradingTimeId(index))(tradingTime).asOpt.value
    }
    ...
    def trusteesCompanyVatVariations(index: Int, reference: ReferenceValue): UserAnswers = {
