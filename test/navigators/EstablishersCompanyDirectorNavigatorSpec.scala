...
class EstablishersCompanyDirectorNavigatorSpec extends SpecBase with NavigatorBehaviour {
  ...
  protected def rowNoValueNewDirector(id: TypedIdentifier.PathDependent)(call: Call): (id.type, UserAnswers, Call) =
    Tuple3(id, newDirector, call)

  protected def rowNewDirector(id: TypedIdentifier.PathDependent)(value: id.Data, call: Call)
    ...
    "in NormalMode" must {
      def navigation: TableFor3[Identifier, UserAnswers, Call] =
        Table(
          ...
          rowNoValueNewDirector(AnyMoreChangesId)(anyMoreChanges(None))
        )
    ...
    "in CheckMode" must {
      def navigation: TableFor3[Identifier, UserAnswers, Call] =
        Table(
          ...
          row(DirectorAddressYearsId(0, 0))(AddressYears.UnderAYear, addressYearsLessThanTwelveEdit(NormalMode, addressYearsUnderAYear, srn = None)),
    ...
    "in UpdateMode" must {
      def navigation: TableFor3[Identifier, UserAnswers, Call] =
        Table(
          ...
          rowNoValueNewDirector(AnyMoreChangesId)(anyMoreChanges(srn))
        )
...
object EstablishersCompanyDirectorNavigatorSpec extends SpecBase with OptionValues {
  ...
  private val confirmPreviousAddressYes = UserAnswers(Json.obj())
    .set(DirectorConfirmPreviousAddressId(0, 0))(true)
    .asOpt
    .value
  private val confirmPreviousAddressNo = UserAnswers(Json.obj())
    .set(DirectorConfirmPreviousAddressId(0, 0))(false)
    .asOpt
    .value
  ...
  private def exitJourney(mode: Mode, answers: UserAnswers, index: Int = 0, srn:Option[String]) =
    if (mode == NormalMode) checkYourAnswers(mode, srn)
      ...
      else anyMoreChanges(None)
    }

  private def hasUtr(ua: UserAnswers, value: Boolean): UserAnswers =
    ua.set(DirectorHasUTRId(0, 0))(value = value).asOpt.value

  private def directorPhone(mode: Mode, srn:Option[String]) = routes.DirectorPhoneNumberController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  private def directorEmail(mode: Mode, srn:Option[String]) = routes.DirectorEmailController.onPageLoad(mode, directorIndex, establisherIndex, srn)
  ...
  private def directorNinoNew(mode: Mode, srn:Option[String]) = routes.DirectorEnterNINOController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  private def directorNinoReason(mode: Mode, srn:Option[String]) = routes.DirectorNoNINOReasonController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  private def directorAddressList(mode: Mode, srn:Option[String]) = routes.DirectorAddressListController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  private def directorAddress(mode: Mode, srn:Option[String]) = routes.DirectorAddressController.onPageLoad(mode, directorIndex, establisherIndex, srn)
  ...
  private def checkYourAnswers(mode: Mode, srn:Option[String]) = routes.CheckYourAnswersController.onPageLoad(directorIndex, establisherIndex, mode, srn)

  private def addCompanyDirectors(mode: Mode, srn:Option[String]) =
    controllers.register.establishers.company.routes.AddCompanyDirectorsController.onPageLoad(mode, None, establisherIndex)
  ...
  private def directorWhyNoUTR(mode: Mode, srn:Option[String]) = routes.DirectorNoUTRReasonController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  private def directorHasUTR(mode: Mode, srn:Option[String]) = routes.DirectorHasUTRController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  private def directorDOB(mode: Mode, srn:Option[String]) = routes.DirectorDOBController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  private def directorHasNINO(mode: Mode, srn:Option[String]) = routes.DirectorHasNINOController.onPageLoad(mode, directorIndex, establisherIndex, srn)

  val addressYearsOverAYearNew = UserAnswers(Json.obj())
    .set(DirectorAddressYearsId(establisherIndex, directorIndex))(AddressYears.OverAYear)
    .flatMap(_.set(IsNewDirectorId(establisherIndex, directorIndex))(true))
    .asOpt
    .value
  ...
  val addressYearsUnderAYearWithPreviousAddress: UserAnswers = UserAnswers(Json.obj())
    .set(DirectorAddressYearsId(establisherIndex, directorIndex))(AddressYears.UnderAYear)
    .flatMap(_.set(ExistingCurrentAddressId(establisherIndex, directorIndex))(Address("Line 1", "Line 2", None, None, None, "UK")))
    .asOpt
    .value
