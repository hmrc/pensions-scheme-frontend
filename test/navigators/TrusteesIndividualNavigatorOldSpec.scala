...
class TrusteesIndividualNavigatorOldSpec extends SpecBase with NavigatorBehaviour {

  import TrusteesIndividualNavigatorOldSpec._
  ...
  private def routes(mode: Mode) = Table(
    ...
    (TrusteeAddressId(0), emptyAnswers, addressYears(mode), true, if(mode == UpdateMode) Some(confirmPreviousAddress) else Some(checkYourAnswers(mode)), true),
  ...
  private val navigator: TrusteesIndividualNavigatorOld =
    new TrusteesIndividualNavigatorOld(FakeUserAnswersCacheConnector, frontendAppConfig)
...
object TrusteesIndividualNavigatorOldSpec extends SpecBase with OptionValues {
