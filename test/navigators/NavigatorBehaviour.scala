...
trait NavigatorBehaviour extends ScalaCheckPropertyChecks with OptionValues {
  ...
  protected def anyMoreChangesPage(srn: OptionalSchemeReferenceNumber = EmptyOptionalSchemeReferenceNumber): Call = AnyMoreChangesController.onPageLoad(srn)

  protected def navigatorWithRoutesForMode(mode: Mode)(navigator: Navigator,
                                                       routes: TableFor3[Identifier, UserAnswers, Call],
                                                       srn: OptionalSchemeReferenceNumber): Unit = {
    forAll(routes) {
