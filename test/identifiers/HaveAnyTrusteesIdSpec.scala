  ...
  "cleanup" when {
    ...
    "`HaveAnyTrustees` changed from Yes to Yes" must {
      ...
      "not remove any data for `Trustees`" in {
        ...
        result.get(HasCompanyNumberId(0)) must be(defined)
        result.get(CompanyRegistrationNumberVariationsId(0)) must be(defined)
        result.get(HasCompanyUTRId(0)) must be(defined)
        result.get(CompanyUTRId(0)) must be(defined)
...
object HaveAnyTrusteesIdSpec extends OptionValues with Enumerable.Implicits {
  ...
  val haveTrusteesYes = UserAnswers(Json.obj())
    ...
    .flatMap(_.set(HasCompanyNumberId(0))(true))
    .flatMap(_.set(CompanyRegistrationNumberVariationsId(0))(ReferenceValue("")))
