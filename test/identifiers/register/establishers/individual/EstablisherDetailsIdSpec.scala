...
class EstablisherDetailsIdSpec extends SpecBase {
  ...
  "cya" when {
    ...
    def answers = UserAnswers().set(EstablisherDetailsId(0))(PersonDetails("test-first-name", None, "test-last-name", now)).asOpt.get
    ...
    "in normal mode individual details" must {
      ...
      "return answers rows with change links" in {
        ...
        EstablisherDetailsId(0).row(onwardUrl, NormalMode) must equal(Seq(
    ...
    "in update mode for new establisher - individual details" must {
      ...
      "return answers rows with change links" in {
        ...
        implicit val userAnswers = request.userAnswers
        EstablisherDetailsId(0).row(onwardUrl, UpdateMode) must equal(Seq(
          AnswerRow("messages__common__cya__name",List("test-first-name test-last-name"),false,
            Some(Link("site.change",onwardUrl,Some("Change test-first-name test-last-name’s name")))),
          AnswerRow("messages__common__dob",List(nowStr),false,
            Some(Link("site.change",onwardUrl,Some("Change test-first-name test-last-name’s date of birth"))))
    ...
    "in update mode for existing establisher - individual details" must {
