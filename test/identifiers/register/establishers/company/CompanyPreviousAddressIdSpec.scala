    ...
    "in update mode" when {
      ...
      "for existing company" must {
        ...
        "return answer row with add link if there is no previous address and `is this previous address` is no" in {
          val answersWithNoIsThisPreviousAddress = UserAnswers().establisherCompanyDetails(Index(0), companyDetails).
        ...
        "return no answer row if there is no previous address and `is this previous address` is yes" in {
          val answersWithYesIsThisPreviousAddress = UserAnswers().establisherCompanyDetails(Index(0), companyDetails).
