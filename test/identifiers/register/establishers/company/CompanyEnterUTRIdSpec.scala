...
class CompanyEnterUTRIdSpec extends SpecBase {
  ...
  "Cleanup" when {
    def answers: UserAnswers =
      UserAnswers(Json.obj())
        .set(CompanyNoUTRReasonId(0))("reason")
        .asOpt
        .value
  ...
  "cya" when {
    ...
    def answers(isEditable: Boolean = false): UserAnswers =
      UserAnswers()
        .set(CompanyEnterUTRId(0))(ReferenceValue(utr, isEditable))
        .flatMap(
          _.set(CompanyDetailsId(0))(CompanyDetails(companyName))
        )
        .asOpt
        .get
