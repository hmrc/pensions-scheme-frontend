...
class CompanyVatIdSpec extends SpecBase {

  "cya" when {
    
    val onwardUrl = "onwardUrl"

    def answers = UserAnswers().set(CompanyVatId(0))(Yes("vat")).asOpt.get
    ...
    "in normal mode" must {
      ...
      "return answers rows with change links" in {
        ...
        CompanyVatId(0).row(onwardUrl, NormalMode) must equal(Seq(
          AnswerRow("messages__company__cya__vat_yes_no",List("site.yes"),true,
            Some(Link("site.change",onwardUrl,Some("messages__visuallyhidden__establisher__vat_yes_no")))),
