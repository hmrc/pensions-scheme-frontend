...
class IsPartnershipDormantIdSpec extends SpecBase with Enumerable.Implicits {

  "cya" when {
    
    val onwardUrl = "onwardUrl"

    def answers = UserAnswers().set(IsPartnershipDormantId(0))(Yes).asOpt.get
    ...
    "in normal mode" must {
      ...
      "return answers rows with change links" in {
        ...
        IsPartnershipDormantId(0).row(onwardUrl, NormalMode) must equal(Seq(
    ...
    "in update mode for new establisher - partnership dormant" must {
    ...
    "in update mode for existing establisher - partnership dormant" must {
