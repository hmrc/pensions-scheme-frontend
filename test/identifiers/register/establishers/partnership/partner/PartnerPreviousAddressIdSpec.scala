...
class PartnerPreviousAddressIdSpec extends WordSpec with MustMatchers with OptionValues with Enumerable.Implicits {
  ...
  "cya" must {
    ...
    val answers = UserAnswers().set(PartnerPreviousAddressId(0, 0))(address).asOpt.value
    ...
    "produce rows with change link for previous address" when {
      "in Normal Mode " in {
        PartnerPreviousAddressId(0, 0).row(onwardUrl) must equal(Seq(
          AnswerRow(
