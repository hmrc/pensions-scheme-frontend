...
class TrusteeDOBIdSpec extends SpecBase {
  ...
  private val answerRowsWithChangeLinks = Seq(
    AnswerRow(messages("messages__individualDOB__heading", "Test Name"),List(DateHelper.formatDate(date)), false, Some(Link("site.change",onwardUrl,
    ...
    "in update mode for existing trustee" must {
      ...
      "return answers rows without change links" in {
        ...
        TrusteeDOBId(0).row(onwardUrl, UpdateMode) must equal(Seq(
          AnswerRow(messages("messages__individualDOB__heading", "Test Name"), List(DateHelper.formatDate(date)),false, None)
        ))
      }
    }
  }
