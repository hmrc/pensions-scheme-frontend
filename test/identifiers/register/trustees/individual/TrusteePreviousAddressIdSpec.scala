...
class TrusteePreviousAddressIdSpec extends WordSpec with MustMatchers with OptionValues with Enumerable.Implicits {

  "cya" must {
    val address = Address("address1", "address2", Some("address3"), Some("address4"), Some("postcode"), "GB")
    val answers = UserAnswers().set(TrusteePreviousAddressId(0))(address).asOpt.value
    ...
    "produce rows with change link for previous address" when {
      "in Normal Mode " in {
        TrusteePreviousAddressId(0).row(onwardUrl) must equal(Seq(
          AnswerRow(
