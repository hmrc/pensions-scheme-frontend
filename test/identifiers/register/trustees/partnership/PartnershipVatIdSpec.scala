...
class PartnershipVatIdSpec extends SpecBase {

  "cya" when {
    
    val onwardUrl = "onwardUrl"

    def answers = UserAnswers().set(PartnershipVatId(0))(Yes("vat")).asOpt.get
    ...
    "in normal mode" must {
      ...
      "return answers rows with change links" in {
        ...
        PartnershipVatId(0).row(onwardUrl, NormalMode) must equal(Seq(
    ...
    "in update mode for new trustee - partnership vat" must {
      ...
      "return answers rows with change links" in {
        ...
        PartnershipVatId(0).row(onwardUrl, UpdateMode) must equal(Seq(
          AnswerRow("messages__common__cya__vat",List("vat"),false,
