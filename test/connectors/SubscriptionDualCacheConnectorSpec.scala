...
class SubscriptionDualCacheConnectorSpec extends AsyncWordSpec with MustMatchers with OptionValues with RecoverMethods {
  ...
  "On toggle On" when {
    ...
    ".removeAll is called" must {
      "removes all the data from old collection when the old collection has data and no data in psa collection" in {
        val connector = new SubscriptionDualCacheConnector(
          frontendAppConfig,
          fakeOldSchemeCacheConnector(true),
          fakeNewSchemeCacheConnector(false),
        ...
        contentAsString(result) mustBe "old remove all"
      }

      "removes all the data from new collection when the new collection has data and no data in old collection" in {
        val connector = new SubscriptionDualCacheConnector(
          frontendAppConfig,
          fakeOldSchemeCacheConnector(false),
          fakeNewSchemeCacheConnector(true),
        ...
        val result = connector.removeAll("foo")
        status(result) mustBe OK
        contentAsString(result) mustBe "new remove all"
      }
    ...
    ".upsert is called" must {
      ...
      "upsert the data from old collection when the old collection has data and no data in psa collection" in {
        val connector = new SubscriptionDualCacheConnector(
          frontendAppConfig,
          fakeOldSchemeCacheConnector(true),
          fakeNewSchemeCacheConnector(false),
        ...
        connector.upsert("foo", Json.obj()) map {
          result =>
            result mustBe Json.obj("data" -> "old upsert")
        }
      }

      "upsert the data from new collection when the new collection has data and no data in old collection" in {
        val connector = new SubscriptionDualCacheConnector(
          frontendAppConfig,
          fakeOldSchemeCacheConnector(false),
          fakeNewSchemeCacheConnector(true),
        ...
        connector.upsert("foo", Json.obj()) map {
          result =>
            result mustBe Json.obj("data" -> "new upsert")
        }
      }
  ...
  "On toggle Off" when {
    ...
    ".removeAll is called" must {
      ...
      "remove all data from old collection" in {
        val connector = new SubscriptionDualCacheConnector(
          frontendAppConfig,
          fakeOldSchemeCacheConnector(true),
          fakeNewSchemeCacheConnector(true),
    ...
    ".upsert is called" must {
      ...
      "upsert data to old collection" in {
        val connector = new SubscriptionDualCacheConnector(
          frontendAppConfig,
          fakeOldSchemeCacheConnector(true),
          fakeNewSchemeCacheConnector(true),
...
object SubscriptionDualCacheConnectorSpec extends SpecBase {
  ...
  private def fakeOldSchemeCacheConnector(isDataExist: Boolean) = new OldSubscriptionCacheConnector(frontendAppConfig, fakeWsClient) {
  ...
  private def fakeNewSchemeCacheConnector(isDataExist: Boolean) = new SubscriptionCacheConnector(frontendAppConfig, fakeWsClient) {
    ...
    override def removeAll(id: String)(implicit ec: ExecutionContext, hc: HeaderCarrier): Future[Result] = Future.successful(Ok("new remove all"))

    override def upsert(cacheId: String, value: JsValue)(implicit ec: ExecutionContext, hc: HeaderCarrier): Future[JsValue] =
      Future.successful(Json.obj("data" -> "new upsert"))
  }
