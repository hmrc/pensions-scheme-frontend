...
import models.{NormalMode, Reference}
    ...
    "return a successful result when there is an existing answer" in {
      ...
      running(_.overrides(
        ...
        app =>
          ...
          val messages = app.injector.instanceOf[MessagesApi].preferred(request)
          val controller = app.injector.instanceOf[TestController]
          val answers = UserAnswers().set(FakeIdentifier)(Reference("123456789")).get
          val result = controller.onPageLoad(viewmodel, answers)

          status(result) mustEqual OK
          contentAsString(result) mustEqual vatVariations(
            appConfig,
            formProvider(companyName)(messages).fill(Reference("123456789")),
            viewmodel,
            None
    ...
    "return a redirect when the submitted data is valid" in {
      ...
      running(_.overrides(
        ...
        app =>
          ...
          FakeUserAnswersService.verify(FakeIdentifier, Reference("123456789", isEditable = true))
      }
    }
...
object VatVariationsControllerSpec {
  ...
  object FakeIdentifier extends TypedIdentifier[Reference]
