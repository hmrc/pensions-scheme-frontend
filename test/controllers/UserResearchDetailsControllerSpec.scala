...
class UserResearchDetailsControllerSpec extends ControllerSpecBase with ScalaFutures {
  ...
  "UserResearchContactDetails Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Json.obj(UserResearchDetailsId.toString -> UserResearchDetails("testName", "test@email.com"))
      val getRelevantData = new FakeDataRetrievalAction(Some(validData))

      val result = controller(getRelevantData).onPageLoad()(fakeRequest)

      contentAsString(result) mustBe viewAsString(form.fill(UserResearchDetails("testName", "test@email.com")))
    }
