...
class DirectorsAlsoTrusteesControllerSpec extends ControllerSpecBase with BeforeAndAfterEach with MockitoSugar {
  ...
  private val postCall = controllers.register.trustees.routes.DirectorsAlsoTrusteesController.onSubmit(0, srn)
  ...
  "onPageLoad when only one establisher" must {
    "return Ok and the correct view on a GET request" in {
      ...
      running(_.overrides(allModules: _*)) { app =>
        ...
        val result = controller.onPageLoad(index, srn)(fakeRequest)
        ...
        contentAsString(result) mustBe
          view(form, Some(schemeName), pageHeading, titleMessage, DataPrefillRadio.radios(seqOneEstablisherDirector), postCall, srn)(fakeRequest, messages).toString
      }
    }
    ...
    "return Ok and redirect to trustee name page on a GET request when feature toggle switched off" in {
      ...
      running(_.overrides(allModules: _*)) { app =>
        ...
        redirectLocation(result) mustBe
          Some(controllers.register.trustees.individual.routes.TrusteeNameController.onPageLoad(NormalMode, 0, srn).url)
      }
    }
  ...
  "onSubmit" must {
    "behave correctly when two directors out of three ticked and None not ticked" in {
      ...
      running(_.overrides(allModules: _*)) { app =>
        ...
        val result = controller.onSubmit(index, srn)(request)
        status(result) mustBe SEE_OTHER
    ...
    "behave correctly when three directors and item None is chosen" in {
      ...
      running(_.overrides(allModules: _*)) { app =>
        ...
        val result = controller.onSubmit(index, srn)(request)
        status(result) mustBe SEE_OTHER
        redirectLocation(result) mustBe Some(controllers.register.trustees.individual.routes.TrusteeNameController.onPageLoad(NormalMode, 0, srn).url)
    ...
    "return bad request when field not filled in" in {
      ...
      running(_.overrides(allModules: _*)) { app =>
        val controller = app.injector.instanceOf[DirectorsAlsoTrusteesController]
        val result = controller.onSubmit(index, srn)(fakeRequest)
