...
class WhatYouWillNeedCompanyContactDetailsControllerSpec extends ControllerSpecBase {
  ...
  private def onwardRoute(mode: Mode, srn: OptionalSchemeReferenceNumber): Call = CompanyEmailController.onPageLoad(mode, index, OptionalSchemeReferenceNumber(srn))
