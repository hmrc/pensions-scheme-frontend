...
class CompanyAddressYearsControllerSpec extends ControllerSpecBase {
  ...
  val viewmodel = AddressYearsViewModel(
    postCall = routes.CompanyAddressYearsController.onSubmit(NormalMode, firstIndex, EmptyOptionalSchemeReferenceNumber),
  ...
  "AddressYears Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val result = controller(getData).onPageLoad(NormalMode, firstIndex, EmptyOptionalSchemeReferenceNumber)(fakeRequest)
    ...
    "redirect to session expired from a GET when the index is invalid" in {
      val result = controller(getRelevantData).onPageLoad(NormalMode, invalidIndex, EmptyOptionalSchemeReferenceNumber)(fakeRequest)
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val result = controller(getRelevantData).onSubmit(NormalMode, firstIndex, EmptyOptionalSchemeReferenceNumber)(postRequest)
      status(result) mustBe SEE_OTHER
