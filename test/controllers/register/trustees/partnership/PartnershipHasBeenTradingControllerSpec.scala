...
class PartnershipHasBeenTradingControllerSpec extends ControllerSpecBase {
  ...
  "PartnershipHasBeenTradingController" must {
    ...
    "clean up previous address, if user changes answer from yes to no" in {
      ...
      val result = controller(getTrusteePartnershipDataWithPreviousAddress(hasBeenTrading = true)).onSubmit(NormalMode, index, srn)(postRequest)
    ...
    "not clean up for previous address, if user changes answer from no to yes" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", "true"))
      val result = controller(getTrusteePartnershipDataWithPreviousAddress(hasBeenTrading = false)).onSubmit(NormalMode, index, srn)(postRequest)
