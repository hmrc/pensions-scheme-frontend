  ...
  "CheckYourAnswersController" when {
    ...
    "on Page load if toggle on in UpdateMode" must {
      "return OK and the correct view for vat if not new trustee" in {
        ...
        val result = controller(answers.dataRetrievalAction, isToggleOn = true).onPageLoad(UpdateMode, firstIndex, None)(userAnswers)
      ...
      "return OK and the correct view for vat if new trustee" in {
        val answers = UserAnswers().set(PartnershipVatVariationsId(firstIndex))("098765432").flatMap(
          _.set(PartnershipPayeVariationsId(firstIndex))("12345678")).flatMap(
          _.set(IsTrusteeNewId(firstIndex))(true)).asOpt.value
        implicit val userAnswers = FakeDataRequest(answers)
        val expectedPartnershipDetailsSection = partnershipDetailsSection(
          PartnershipVatId(firstIndex).row(partnershipVatRoute, UpdateMode)++
          PartnershipPayeId(firstIndex).row(partnershipPayeRoute, UpdateMode)
        )
        val result = controller(answers.dataRetrievalAction, isToggleOn = true).onPageLoad(UpdateMode, firstIndex, None)(userAnswers)
    ...
    "on Submit" must {
      ...
      "mark trustee partnership as complete" in {
        val result = controller().onSubmit(NormalMode, firstIndex, None)(fakeRequest)
        status(result) mustBe SEE_OTHER
