...
class TrusteeAddressControllerSpec extends ControllerSpecBase with MockitoSugar with ScalaFutures with CSRFRequest {
  ...
  "TrusteeAddressController" must {
      ...
      running(_.overrides(
        ...
        implicit app =>
          ...
          val request = addToken(
            FakeRequest(TrusteeAddressController.onPageLoad(NormalMode, firstIndex))
    ...
    "redirect to next page on POST request" which {
      "save address" in {
        ...
        def onwardCall = routes.TrusteeAddressController.onPageLoad(NormalMode,Index(0))
        ...
        running(_.overrides(
          ...
          bind[DataCacheConnector].toInstance(FakeDataCacheConnector),
          bind[AuthAction].to(FakeAuthAction),
          bind[Navigator].to(new FakeNavigator(onwardCall, NormalMode)),
          bind[DataRetrievalAction].to(retrieval),
          ...
          implicit app =>
            ...
            val fakeRequest = addToken(FakeRequest(TrusteeAddressController.onSubmit(NormalMode, firstIndex))
              .withHeaders("Csrf-Token" -> "nocheck")
            ...
            FakeDataCacheConnector.verify(TrusteeAddressId(firstIndex), address)
        }
      }
    }
