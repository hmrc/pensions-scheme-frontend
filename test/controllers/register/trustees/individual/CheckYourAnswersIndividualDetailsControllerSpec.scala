...
object CheckYourAnswersIndividualDetailsControllerSpec extends ControllerSpecBase with Enumerable.Implicits
  ...
  private def allAddLinksVariations(implicit request: DataRequest[AnyContent]): Seq[AnswerSection] =
    Seq(AnswerSection(
      None,
      Seq(
        stringLink(messages("messages__individualDOB__heading", name), DateHelper.formatDate(trusteeDob)),
  ...
  private def allValuesYes(mode: Mode, srn: Option[String]
    ...
    Seq(AnswerSection(
      None,
      Seq(
        stringChangeLink(messages("messages__individualDOB__heading", name), trusteeDob(mode, srn), DateHelper.formatDate(trusteeDob),
