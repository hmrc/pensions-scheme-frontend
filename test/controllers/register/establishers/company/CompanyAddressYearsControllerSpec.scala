...
class CompanyAddressYearsControllerSpec extends ControllerSpecBase {
  ...
  private def viewAsString(form: Form[_] = form) =
    addressYears(
      ...
      AddressYearsViewModel(
        ...
        Message("messages__company_address_years__title", Message("messages__common__address_years__company").resolve),
        Message("messages__company_address_years__title", companyName),
        Message("messages__company_address_years__title", companyName),
        Some(companyName)
      ),
      None
  ...
  "CompanyAddressYears Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val result = controller(getRelevantData).onSubmit(NormalMode, None, firstIndex)(postRequest)
      status(result) mustBe SEE_OTHER
