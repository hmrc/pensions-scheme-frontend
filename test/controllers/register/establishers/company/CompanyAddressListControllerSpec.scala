...
class CompanyAddressListControllerSpec extends ControllerSpecBase with OptionValues {
  ...
  private val onwardRoute = controllers.register.establishers.company.routes.CompanyAddressYearsController.onPageLoad(NormalMode, srn, 0)
  ...
  "Company Address List Controller" must {
    ...
    "redirect to Company Address Post Code Lookup if no address data on a GET request" in {
      ...
      running(_.overrides(modules(UserAnswers().dataRetrievalAction): _*)) { app =>
        ...
        redirectLocation(result) mustBe Some(routes.CompanyPostCodeLookupController.onPageLoad(NormalMode, srn, Index(0)).url)
      }
    }
  ...
  private def addressListViewModel(addresses: Seq[TolerantAddress]): AddressListViewModel = {
    AddressListViewModel(
      routes.CompanyAddressListController.onSubmit(NormalMode, srn, Index(0)),
      routes.CompanyAddressController.onPageLoad(NormalMode, srn, Index(0)),
      addresses,
