...
class CompanyPreviousAddressListControllerSpec extends ControllerSpecBase {
  ...
  def viewAsString(form: Form[?] = form): String =
    view(
      ...
      AddressListViewModel(
        routes.CompanyPreviousAddressListController.onSubmit(NormalMode, EmptyOptionalSchemeReferenceNumber, index),
  ...
  "CompanyPreviousAddressList Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val result = controller(getData).onPageLoad(NormalMode, EmptyOptionalSchemeReferenceNumber, index)(fakeRequest)
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val getData = new FakeDataRetrievalAction(Some(validData))
      val result = controller(getData).onSubmit(NormalMode, EmptyOptionalSchemeReferenceNumber, index)(postRequest)
    ...
    "redirect to postcode lookup when no address results exist (get)" in {
      ...
      redirectLocation(result).value mustEqual
        controllers.register.establishers.company.routes.CompanyPreviousAddressPostcodeLookupController.onPageLoad(NormalMode, EmptyOptionalSchemeReferenceNumber, 0).url
    }

    "redirect to postcode lookup when no address results exist (post)" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", "0"))
      val result = controller().onSubmit(NormalMode, EmptyOptionalSchemeReferenceNumber, 0)(postRequest)
      status(result) mustEqual SEE_OTHER
      redirectLocation(result).value mustEqual
        controllers.register.establishers.company.routes.CompanyPreviousAddressPostcodeLookupController.onPageLoad(NormalMode, EmptyOptionalSchemeReferenceNumber, 0).url
    }
