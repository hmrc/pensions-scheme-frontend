...
import identifiers.register.establishers.company.{CompanyDetailsId, DoesCompanyHavePAYENoId}
...
class DoesCompanyHavePAYENoControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getMandatoryEstablisherCompany): DoesCompanyHavePAYENoController =
    new DoesCompanyHavePAYENoController(
      frontendAppConfig,
      messagesApi,
  ...
  def postCall: Call = routes.DoesCompanyHavePAYENoController.onSubmit(NormalMode, None, index)
  ...
  val validData: JsObject = Json.obj(
      EstablishersId.toString -> Json.arr(
        Json.obj(
          ...
          DoesCompanyHavePAYENoId.toString -> true
        )
      )
    )

  def viewAsString(form: Form[_] = form): String = doesCompanyHavePAYENumber(frontendAppConfig, form, companyName, postCall, None)(fakeRequest, messages).toString

  "DoesCompanyHavePAYENo Controller" must {
