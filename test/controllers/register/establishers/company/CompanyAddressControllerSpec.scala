...
class CompanyAddressControllerSpec extends ControllerSpecBase with ScalaFutures {
  ...
  def viewAsString(form: Form[?] = form): String =
    view(
      ...
      ManualAddressViewModel(
        routes.CompanyAddressController.onSubmit(NormalMode, EmptyOptionalSchemeReferenceNumber, firstIndex),
        options,
  ...
  "CompanyAddress Controller" must {
    ...
    "send an audit event when valid data is submitted" in {
      ...
      val result = controller(data).onSubmit(NormalMode, EmptyOptionalSchemeReferenceNumber, firstIndex)(postRequest)
