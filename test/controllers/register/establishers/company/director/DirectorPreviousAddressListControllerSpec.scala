...
class DirectorPreviousAddressListControllerSpec extends ControllerSpecBase {
    ...
    "redirect to Director Previous Address Post Code Lookup if no address data on a GET request" in {
      ...
      running(_.overrides(modules(UserAnswers().dataRetrievalAction): _*)) { app =>
        ...
        redirectLocation(result) mustBe Some(
          routes.DirectorPreviousAddressPostcodeLookupController.onPageLoad(NormalMode, establisherIndex = 0, directorIndex = 0,srn).url)
      }
    }
  ...
  private def addressListViewModel(addresses: Seq[TolerantAddress]): AddressListViewModel = {
    AddressListViewModel(
      routes.DirectorPreviousAddressListController.onSubmit(NormalMode, Index(0), Index(0), srn),
      routes.DirectorPreviousAddressController.onPageLoad(NormalMode, Index(0), Index(0), srn),
      addresses,
