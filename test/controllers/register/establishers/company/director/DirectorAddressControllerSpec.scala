...
class DirectorAddressControllerSpec extends ControllerSpecBase with MockitoSugar with ScalaFutures with OptionValues {
  ...
  private val onwardCall = routes.DirectorAddressYearsController.onPageLoad(NormalMode, establisherIndex, directorIndex, srn)
  ...
  "Address Controller" must {
    ...
    "render manualAddress from GET request" in {
      val postCall = routes.DirectorAddressController.onSubmit(NormalMode, Index(establisherIndex), Index(directorIndex), srn)
    ...
    "redirect to next page on POST request" which {
      "saves director address" in {
        val onwardCall = routes.DirectorAddressYearsController.onPageLoad(NormalMode, establisherIndex, directorIndex, srn)
        running(_.overrides(modules(retrieval) ++
