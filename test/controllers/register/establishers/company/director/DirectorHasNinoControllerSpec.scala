...
class DirectorHasNINOControllerSpec extends ControllerSpecBase {
  ...
  val viewModel = CommonFormWithHintViewModel(
    ...
    heading = Message("messages__directorHasNino__h1", "test company name"),
    hint = None
  )

  def controller(dataRetrievalAction: DataRetrievalAction = getMandatoryEstablisherCompany): DirectorHasNINOController =
