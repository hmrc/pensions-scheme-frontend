...
class CompanyPreviousAddressControllerSpec extends ControllerSpecBase with ScalaFutures {
  ...
  def viewAsString(form: Form[_] = form): String =
    view(
      ...
      ManualAddressViewModel(
        routes.CompanyPreviousAddressController.onSubmit(NormalMode, EmptyOptionalSchemeReferenceNumber, index),
        options,
  ...
  "CompanyPreviousAddress Controller" must {
    ...
    "send an audit event when valid data is submitted" in {
      ...
      val result = controller(data).onSubmit(NormalMode, EmptyOptionalSchemeReferenceNumber, index)(postRequest)
