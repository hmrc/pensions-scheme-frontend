...
class CompanyPreviousAddressControllerSpec extends ControllerSpecBase with ScalaFutures {
  ...
  def viewAsString(form: Form[_] = form): String =
    view(
      ...
      ManualAddressViewModel(
        routes.CompanyPreviousAddressController.onSubmit(NormalMode, srn, index),
        options,
        Message("messages__common__confirmPreviousAddress__h1", Message("messages__theEstablisher")),
        Message("messages__common__confirmPreviousAddress__h1", companyName), srn
      ),
      None
  ...
  "CompanyPreviousAddress Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val result = controller(getRelevantData).onPageLoad(NormalMode, srn, index)(fakeRequest)
    ...
    "send an audit event when valid data is submitted" in {
      ...
      val result = controller(data).onSubmit(NormalMode, srn, index)(postRequest)
