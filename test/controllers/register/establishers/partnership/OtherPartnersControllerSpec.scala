...
class OtherPartnersControllerSpec extends ControllerSpecBase {
  ...
  val submitUrl = controllers.register.establishers.partnership.routes.OtherPartnersController.onSubmit(NormalMode, index, srn)
  ...
  "OtherPartners Controller" must {
    ...
    "redirect to session expired page on a GET when the index is not valid" in {
      val getRelevantData = new FakeDataRetrievalAction(Some(validData))
      val result = controller(getRelevantData).onPageLoad(NormalMode, invalidIndex, srn)(fakeRequest)
