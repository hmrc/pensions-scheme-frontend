...
class ConfirmDeletePartnerControllerSpec extends ControllerSpecBase {
  ...
  "ConfirmDeletePartner Controller" must {
    "return OK and the correct view for a GET" in {
      ...
      val result = controller(data).onPageLoad(NormalMode, establisherIndex, partnerIndex, srn)(fakeRequest)
    ...
    "redirect to already deleted view for a GET if the partner was already deleted" in {
      ...
      redirectLocation(result) mustBe Some(routes.AlreadyDeletedController.onPageLoad(NormalMode, establisherIndex, partnerIndex, srn).url)
    }
    ...
    "return a Bad Request on POST and errors when invalid data is submitted" in {
      ...
      val result = controller(data).onSubmit(NormalMode, establisherIndex, partnerIndex, srn)(postRequest)
    ...
    "never delete the partner on a POST if selected No" in {
      ...
      val data = new FakeDataRetrievalAction(Some(testData()))
      val result = controller(data).onSubmit(NormalMode, establisherIndex, partnerIndex, srn)(postRequestForCancle)
    ...
    "never delete the partner on a POST if selected No in UpdateMode" in {
      FakeUserAnswersService.reset()
      val data = new FakeDataRetrievalAction(Some(testData()))
      val result = controller(data).onSubmit(UpdateMode, establisherIndex, partnerIndex, srn)(postRequestForCancle)
    ...
    "redirect to Session Expired for a GET if no existing data is found" in {
      val result = controller(dontGetAnyData).onPageLoad(NormalMode, establisherIndex, partnerIndex, srn)(fakeRequest)
    ...
    "redirect to Session Expired for a POST if no existing data is found" in {
      val result = controller(dontGetAnyData).onSubmit(NormalMode, establisherIndex, partnerIndex, srn)(postRequest)
...
object ConfirmDeletePartnerControllerSpec extends ControllerSpecBase {
  ...
  private lazy val postCall = routes.ConfirmDeletePartnerController.onSubmit(NormalMode, establisherIndex, partnerIndex, srn)
