...
class PartnerDOBControllerSpec extends ControllerSpecBase with DateOfBirthControllerBehaviours {
  ...
  private val postCall: (Mode, Index, Index, SchemeReferenceNumber) => Call = routes.PartnerDOBController.onSubmit

  private def viewModel(mode: Mode, establisherIndex: Index, partnerIndex: Index, srn: SchemeReferenceNumber, token: String): DateOfBirthViewModel =
  ...
  "PartnerDOB Controller" must {
    ...
    behave like dateOfBirthController(
      get = data => controller(data).onPageLoad(NormalMode, firstEstablisherIndex, firstPartnerIndex, srn),
      post = data => controller(data).onSubmit(NormalMode, firstEstablisherIndex, firstPartnerIndex, srn),
      viewModel = viewModel(NormalMode, firstEstablisherIndex, firstPartnerIndex, srn, Message("messages__thePartner")),
      mode = NormalMode,
