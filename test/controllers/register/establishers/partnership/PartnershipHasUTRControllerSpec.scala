...
class PartnershipHasUTRControllerSpec extends ControllerSpecBase {
  ...
  private val viewModel = CommonFormWithHintViewModel(
    postCall = routes.PartnershipHasUTRController.onSubmit(NormalMode, index, srn),
    title = Message("messages__partnershipHasUtr__title"),
    heading = Message("messages__partnershipHasUtr__heading", partnershipDetails.name),
  ...
  "PartnershipHasUTRController" when {
    "on a GET" must {
      ...
      "return OK and the correct view where question already answered" in {
        running(_.overrides(modules(fullAnswers.set(PartnershipHasUTRId(index))(value = false).asOpt.value.dataRetrievalAction,
