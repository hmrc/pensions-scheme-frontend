...
class AlreadyDeletedControllerSpec extends ControllerSpecBase {
  ...
  def onwardRoute: Call = controllers.register.establishers.routes.AddEstablisherController.onPageLoad(NormalMode)
  ...
  "AlreadyDeleted Establisher Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      val result = controller().onPageLoad(establisherIndex, EstablisherKind.Indivdual)(fakeRequest)
    ...
    "redirect to Session Expired for a GET if no existing data is found" in {
      val result = controller(dontGetAnyData).onPageLoad(establisherIndex, EstablisherKind.Indivdual)(fakeRequest)
