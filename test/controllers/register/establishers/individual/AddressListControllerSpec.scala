...
class AddressListControllerSpec extends ControllerSpecBase with Enumerable.Implicits with MapFormats with MockitoSugar {
  ...
  def viewAsString(form: Form[_] = form, address: Seq[TolerantAddress] = addresses): String =
    view(
      ...
      AddressListViewModel(
        postCall = routes.AddressListController.onSubmit(NormalMode, firstIndex, srn),
        manualInputCall = routes.AddressController.onPageLoad(NormalMode, firstIndex, srn),
        addresses = addresses,
  ...
  "AddressResults Controller" must {
    ...
    "redirect to Address look up page when no addresses are present after lookup" in {
      val result = controller(new FakeDataRetrievalAction(Some(validDataNoAddresses))).onPageLoad(NormalMode, firstIndex, srn)(fakeRequest)

      status(result) mustBe SEE_OTHER
      redirectLocation(result) mustBe Some(
        controllers.register.establishers.individual.routes.PostCodeLookupController.onPageLoad(NormalMode, firstIndex, srn).url)
    }
