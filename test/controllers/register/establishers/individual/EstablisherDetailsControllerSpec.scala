...
class EstablisherDetailsControllerSpec extends ControllerSpecBase {
  ...
  def viewAsString(form: Form[_] = form): String =
    establisherDetails(frontendAppConfig, form, NormalMode, Index(1), schemeName)(fakeRequest, messages).toString
  ...
  "EstablisherDetails Controller" must {
    ...
    "redirect to session expired page on a GET when scheme name is not present" in {
      val result = controller(getEmptyCacheMap).onPageLoad(NormalMode, Index(1))(fakeRequest)
