...
import models.{AddressYears, EmptyOptionalSchemeReferenceNumber, Index, NormalMode, UpdateMode}
...
class AddressYearsControllerSpec extends ControllerSpecBase {
  ...
  private lazy val viewModel =
    AddressYearsViewModel(
      postCall = routes.AddressYearsController.onSubmit(NormalMode, firstIndex, EmptyOptionalSchemeReferenceNumber),
  ...
  "AddressYears Controller" must {
    ...
    "redirect to the next page when valid data is submitted with over_a_year" in {
      ...
      val result = controller().onSubmit(UpdateMode, firstIndex, EmptyOptionalSchemeReferenceNumber)(postRequest)
      status(result) mustBe SEE_OTHER
