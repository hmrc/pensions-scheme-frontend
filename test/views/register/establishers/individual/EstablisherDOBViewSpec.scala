...
class EstablisherDOBViewSpec extends QuestionViewBehaviours[LocalDate] {

  val messageKeyPrefix = "individualDOB"

  val index = Index(1)
  val establisherName = "John Doe"
  private val postCall = routes.EstablisherDOBController.onSubmit _

  override val form = new DOBFormProvider()()

  def createView(): () => HtmlFormat.Appendable = () =>
    establisherDOB(frontendAppConfig, form, NormalMode, None,
      postCall(NormalMode, index, None), None, establisherName)(fakeRequest, messages)

  def createUpdateView(): () => HtmlFormat.Appendable = () =>
    establisherDOB(frontendAppConfig, form, NormalMode, None,
      postCall(NormalMode, index, None), Some("srn"), establisherName)(fakeRequest, messages)
  ...
  "Establisher DOB view" must {

    behave like normalPageWithTitle(createView(), messageKeyPrefix,
      messages("messages__individualDOB__heading", messages("messages__theEstablisher")), messages(s"messages__${messageKeyPrefix}__heading", establisherName))
