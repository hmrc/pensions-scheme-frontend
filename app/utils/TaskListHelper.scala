...
class TaskListHelper(journey: Option[UserAnswers])(implicit messages: Messages) extends Enumerable.Implicits {
  ...
  private def workingKnowledgeSection(implicit userAnswers: UserAnswers) = {
    ...
    val link = if (isComplete.getOrElse(false)) {
      ...
      Link(workingKnowledgeLinkText, controllers.routes.WhatYouWillNeedWorkingKnowledgeController.onPageLoad.url)
    }
  ...
  private[utils] def listOf(sections: Seq[Entity[_]])(implicit userAnswers: UserAnswers): Seq[JourneyTaskListSection] = {
