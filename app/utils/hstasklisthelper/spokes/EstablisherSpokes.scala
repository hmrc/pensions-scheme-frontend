...
case object EstablisherCompanyDetails extends DetailsSpoke {
  override def addLinkUrl(mode: Mode, srn: SchemeReferenceNumber, index: Option[Index]): Call =
    establisherCompanyRoutes.WhatYouWillNeedCompanyDetailsController.onPageLoad(mode, srn, index.getOrElse(Index(0)))

  override def changeLinkUrl(mode: Mode, srn: SchemeReferenceNumber, index: Option[Index]): Call =
