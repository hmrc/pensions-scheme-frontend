...
case object WorkingKnowledgeSpoke extends Spoke {
  override def addLink(name: String)(mode: Mode, srn: Option[String], index: Option[Index]): TaskListLink =
    TaskListLink(
    "messages__schemeTaskList__add_details_wk", controllers.routes.WhatYouWillNeedWorkingKnowledgeController.onPageLoad().url
  )

  override def changeLink(name: String)(mode: Mode, srn: Option[String], index: Option[Index]): TaskListLink =
    TaskListLink(
      "messages__schemeTaskList__change_details", controllers.routes.AdviserCheckYourAnswersController.onPageLoad().url
    )

  override def incompleteChangeLink(name: String)(mode: Mode, srn: Option[String], index: Option[Index]): TaskListLink =
    TaskListLink(
      "messages__schemeTaskList__change_details", controllers.routes.WhatYouWillNeedWorkingKnowledgeController.onPageLoad().url
  )
