...
import views.html.hasBeenTrading

import scala.concurrent.Future

trait HasBeenTradingController extends FrontendController with Retrievals with I18nSupport {
  ...
  def get(id: TypedIdentifier[Boolean], form: Form[Boolean], viewmodel: CommonFormWithHintViewModel)
    ...
    Future.successful(Ok(hasBeenTrading(appConfig, preparedForm, viewmodel, existingSchemeName)))
  }
  ...
  def post(id: TypedIdentifier[Boolean], mode: Mode, form: Form[Boolean], viewmodel: CommonFormWithHintViewModel)
    ...
    form.bindFromRequest().fold(
      (formWithErrors: Form[_]) =>
        Future.successful(BadRequest(hasBeenTrading(appConfig, formWithErrors, viewmodel, existingSchemeName))),
      value =>
