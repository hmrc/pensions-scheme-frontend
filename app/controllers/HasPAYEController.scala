...
trait HasPAYEController extends FrontendController with Retrievals with I18nSupport {
  ...
  def get(id: TypedIdentifier[Boolean], form: Form[Boolean], viewmodel: CommonFormWithHintViewModel)
    ...
    Future.successful(Ok(hasPAYE(appConfig, preparedForm, viewmodel, existingSchemeName)))
  }
  ...
  def post(id: TypedIdentifier[Boolean], mode: Mode, form: Form[Boolean], viewmodel: CommonFormWithHintViewModel)
    ...
    form.bindFromRequest().fold(
      (formWithErrors: Form[_]) =>
        Future.successful(BadRequest(hasPAYE(appConfig, formWithErrors, viewmodel, existingSchemeName))),
      value =>
