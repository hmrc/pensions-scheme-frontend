    ...
    (authenticate andThen getData(mode, srn) andThen allowAccess(srn) andThen requireData).async {
      implicit request =>
        val redirectResult = request.userAnswers.get(CompanyRegistrationNumberId(index)) match {
          case None =>
            Ok(companyRegistrationNumber(appConfig, form, Mode.journeyMode(mode), index, existingSchemeName, postCall(mode, srn, index), srn))
          case Some(value) =>
            Ok(companyRegistrationNumber(appConfig, form.fill(value), Mode.journeyMode(mode), index, existingSchemeName, postCall(mode, srn, index), srn))
        }
    ...
    implicit request =>
      ...
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(companyRegistrationNumber(appConfig, formWithErrors, Mode.journeyMode(mode), index, existingSchemeName, postCall(mode, srn, index), srn))),
        value =>
