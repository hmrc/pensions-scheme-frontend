...
import identifiers.register.establishers.company.{CompanyDetailsId, HasCompanyNumberId, IsCompanyCompleteId, IsDetailsCompleteId}
  ...
  def onSubmit(mode: Mode, srn: Option[String] = None, index: Index): Action[AnyContent] =
    (authenticate andThen getData(mode, srn) andThen requireData).async {
      implicit request =>
        CompanyDetailsId(index).retrieve.right.map {
          details =>
            post(HasCompanyNumberId(index), mode, form(details.companyName), viewModel(mode, index, srn, details.companyName), Some(IsCompanyCompleteId(index)))
        }
    }
}
