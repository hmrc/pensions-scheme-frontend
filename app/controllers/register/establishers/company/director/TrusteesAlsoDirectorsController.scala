...
class TrusteesAlsoDirectorsController @Inject()(override val messagesApi: MessagesApi,
  ...
  def onSubmit(mode: Mode, srn: OptionalSchemeReferenceNumber, establisherIndex: Index): Action[AnyContent] =
    (authenticate() andThen getData(mode, srn) andThen allowAccess(EmptyOptionalSchemeReferenceNumber) andThen requireData).async {
      implicit request =>
        ...
        CompanyDetailsId(establisherIndex).retrieve.map { companyName =>
          if (seqTrustee.size > 1) {
            val boundForm: Form[List[Int]] =
              formCheckBox(establisherIndex)(request.userAnswers, implicitly).bindFromRequest()
  ...
  private def formCheckBox(index: Index)
                          (implicit ua: UserAnswers, messages: Messages): Form[List[Int]] = {
