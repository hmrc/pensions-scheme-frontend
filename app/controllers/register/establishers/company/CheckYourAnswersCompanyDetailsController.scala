    ...
    (authenticate andThen getData(mode, srn) andThen allowAccess(srn) andThen requireData).async {
      implicit request =>
        ...
        val companyDetails = Seq(AnswerSection(
          ...
          CompanyVatVariationsId(index).row(routes.CompanyVatVariationsController.onPageLoad(checkMode(mode), srn, index).url, mode) ++
          HasCompanyPAYEId(index).row(routes.HasCompanyPAYEController.onPageLoad(checkMode(mode), srn, index).url, mode) ++
          CompanyPayeVariationsId(index).row(routes.CompanyPayeVariationsController.onPageLoad(checkMode(mode), srn, index).url, mode) ++
        ...
        Future.successful(Ok(check_your_answers(
          appConfig,
          companyDetails,
          routes.CheckYourAnswersCompanyDetailsController.onSubmit(mode, srn, index),
          existingSchemeName,
          mode = mode,
