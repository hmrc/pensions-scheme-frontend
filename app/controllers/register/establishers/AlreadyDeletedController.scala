...
class AlreadyDeletedController @Inject()(
  ...
  def onPageLoad(establisherIndex: Index): Action[AnyContent] = (authenticate andThen getData andThen requireData).async {
    implicit request =>
      retrieveEstablisherName(establisherIndex)(establisher =>
        Future.successful(Ok(alreadyDeleted(appConfig, vm(establisherIndex, establisher))))
      )
  }

  private def vm(establisherIndex: Index, establisherName: String) = AlreadyDeletedViewModel(
