...
class HaveAnyTrusteesController @Inject()(
  ...
  def onPageLoad(mode: Mode, srn: Option[String] = None): Action[AnyContent] = (authenticate andThen getData).async {
    implicit request =>
  ...
  def onSubmit(mode: Mode, srn: Option[String] = None): Action[AnyContent] = (authenticate andThen getData).async {
    implicit request =>
