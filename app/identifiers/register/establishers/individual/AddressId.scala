...
object AddressId {
  ...
  implicit def cya(implicit countryOptions: CountryOptions,
                   messages: Messages,
                   ua: UserAnswers): CheckYourAnswers[AddressId] =
    new CheckYourAnswers[AddressId] {

      override def row(id: AddressId)(changeUrl: String, ua: UserAnswers): Seq[AnswerRow] = {
        val establisherName = ua.get(EstablisherNameId(id.index)).map(_.fullName)
          .getOrElse(messages("messages__theEstablisher"))
      ...
      override def updateRow(id: AddressId)(changeUrl: String, ua: UserAnswers): Seq[AnswerRow] =
        row(id)(changeUrl, ua)
    }
}
