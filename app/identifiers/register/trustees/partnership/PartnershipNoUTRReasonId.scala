...
object PartnershipNoUTRReasonId {
  ...
  implicit def cya(implicit userAnswers: UserAnswers,
    ...
    def trusteeName(index: Int) =
      userAnswers.get(PartnershipDetailsId(index)).fold(messages("messages__theTrustee"))(_.name)
    def label(index: Int) =
      Some(messages("messages__whyNoUTR", trusteeName(index)))
    def hiddenLabel(index: Int) =
      Some(messages("messages__visuallyhidden__dynamic_noUtrReason", trusteeName(index)))
    ...
    new CheckYourAnswers[PartnershipNoUTRReasonId] {
      ...
      override def updateRow(id: PartnershipNoUTRReasonId)(changeUrl: String,
                                                           userAnswers: UserAnswers): Seq[AnswerRow] =
