...
object PartnershipHasPAYEId {
  ...
  implicit def cya(implicit userAnswers: UserAnswers, messages: Messages): CheckYourAnswers[PartnershipHasPAYEId] = {
    ...
    def label(index: Int) = userAnswers.get(PartnershipDetailsId(index)) match {
      case Some(details) => Some(messages("messages__hasPaye__h1", details.name))
      case _ => Some(messages("messages__partnershipHasPaye__title"))
    }

    def hiddenLabel = Some(messages("messages__visuallyhidden__partnership__paye_yes_no"))
