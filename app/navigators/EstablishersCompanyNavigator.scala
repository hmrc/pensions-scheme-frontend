...
class EstablishersCompanyNavigator @Inject()(val dataCacheConnector: UserAnswersCacheConnector,
  ...
  private def psaSchemeTaskListRegistrationEstablisher(index: Index) =
  ...
  protected def routes(from: NavigateFrom, mode: Mode, srn: OptionalSchemeReferenceNumber): Option[NavigateTo] =
    from.id match {
      case CompanyDetailsId(index) =>
        NavigateTo.dontSave(psaSchemeTaskListRegistrationEstablisher(index))
      ...
      case HasCompanyPAYEId(index) =>
        confirmHasCompanyPAYE(index, mode, srn)(from.userAnswers)
  ...
  private def previousAddressEditRoutes(index: Int, mode: Mode, srn: OptionalSchemeReferenceNumber, userAnswers: UserAnswers): Option[NavigateTo] =
  ...
  protected def editRoutes(from: NavigateFrom, mode: Mode, srn: OptionalSchemeReferenceNumber): Option[NavigateTo] =
    from.id match {
      ...
      case CompanyConfirmPreviousAddressId(index) =>
      ...
      if (answers.allDirectorsAfterDelete(index).length < appConfig.maxDirectors) {
        answers.get(AddCompanyDirectorsId(index)).map { addCompanyDirectors =>
          if (addCompanyDirectors) {
            ...
            if (mode == CheckMode || mode == NormalMode) {
              psaSchemeTaskListRegistrationEstablisher(index)
            } else {
