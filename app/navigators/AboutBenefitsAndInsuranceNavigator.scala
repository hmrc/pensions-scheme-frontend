...
class AboutBenefitsAndInsuranceNavigator @Inject()(val dataCacheConnector: UserAnswersCacheConnector, appConfig: FrontendAppConfig) extends AbstractNavigator {
  ...
  override protected def routeMap(from: NavigateFrom): Option[NavigateTo] = {
    from.id match {
      ...
      case InsurerSelectAddressId => checkYourAnswers(NormalMode) //NavigateTo.dontSave(InsurerConfirmAddressController.onPageLoad(NormalMode, None))
  ...
  protected def checkUpdateRouteMap(from: NavigateFrom, srn: Option[String]): Option[NavigateTo] = {
    from.id match {
      ...
      case InsurerSelectAddressId => anyMoreChanges(srn) // NavigateTo.dontSave(InsurerConfirmAddressController.onPageLoad(CheckUpdateMode, srn))
  ...
  private def benefitsSecuredEditRoutes(userAnswers: UserAnswers, mode: Mode, srn: Option[String] = None): Option[NavigateTo] = {
    userAnswers.get(BenefitsSecuredByInsuranceId) match {
