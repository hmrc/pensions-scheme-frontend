...
trait Navigator {
  ...
  def nextPage(id: Identifier, mode: Mode, userAnswers: UserAnswers, srn: Option[SchemeReferenceNumber] = None)
...
abstract class AbstractNavigator
  ...
  protected def updateRouteMap(from: NavigateFrom, srn: Option[SchemeReferenceNumber] = None): Option[NavigateTo]

  protected def checkUpdateRouteMap(from: NavigateFrom, srn: Option[SchemeReferenceNumber] = None): Option[NavigateTo]
  ...
  protected def anyMoreChangesPage(srn: Option[SchemeReferenceNumber]): Call = AnyMoreChangesController.onPageLoad(srn)
