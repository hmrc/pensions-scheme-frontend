...
class PartnerNavigator @Inject()(val dataCacheConnector: UserAnswersCacheConnector,
  ...
  private def addPartnerRoutes(mode: Mode, ua: UserAnswers, estIndex: Int, srn: SchemeReferenceNumber): Call = {
  ...
  private def checkUpdateModeRoute(mode: CheckUpdateMode.type, ua: UserAnswers, srn: SchemeReferenceNumber)
  : PartialFunction[Identifier, Call] = {
    case PartnerNameId(estIndex, partnerIndex) => cyaPage(mode, estIndex, partnerIndex, srn)
    case PartnerDOBId(estIndex, partnerIndex) => cyaPage(mode, estIndex, partnerIndex, srn)
    case id@PartnerHasNINOId(estIndex, partnerIndex) =>
...
object PartnerNavigator {
  ...
  def taskListPage(mode: Mode, srn: SchemeReferenceNumber): Call = controllers.routes.PsaSchemeTaskListController.onPageLoad(mode, srn)
  ...
  private def isThisPaPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerConfirmPreviousAddressController.onPageLoad(estIndex, partnerIndex, srn)

  private def addPartnerPage(mode: Mode, estIndex: Int, srn: SchemeReferenceNumber): Call =
    controllers.register.establishers.partnership.routes.AddPartnersController.onPageLoad(mode, estIndex, srn)

  private def dobPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerDOBController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def hasNinoPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerHasNINOController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def ninoPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerEnterNINOController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def whyNoNinoPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerNoNINOReasonController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def hasUtrPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerHasUTRController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def utrPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerEnterUTRController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def whyNoUtrPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerNoUTRReasonController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def postcodeLookupPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerAddressPostcodeLookupController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def addressListPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerAddressListController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def paAddressListPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerPreviousAddressListController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def addressYearsPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerAddressYearsController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def phonePage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
  ...
  private def partnerAddressYearsRoutes(mode: Mode, ua: UserAnswers, estIndex: Int, partnerIndex: Int,
                                        srn: SchemeReferenceNumber): Call =
  ...
  private def paPostcodeLookupPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
    PartnerPreviousAddressPostcodeLookupController.onPageLoad(mode, estIndex, partnerIndex, srn)

  private def emailPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
  ...
  private def cyaPage(mode: Mode, estIndex: Int, partnerIndex: Int, srn: SchemeReferenceNumber): Call =
