...
class EstablishersIndividualDetailsNavigator @Inject()(val dataCacheConnector: UserAnswersCacheConnector) extends AbstractNavigator {
  ...
  private def checkUpdateModeRoutes(mode: CheckUpdateMode.type, ua: UserAnswers, srn: Option[String]): PartialFunction[Identifier, Call] = {
    case EstablisherDOBId(index)                                                           => CheckYourAnswersDetailsController.onPageLoad(journeyMode(mode), index, srn)
    case EstablisherEnterNINOId(index) if ua.get(IsEstablisherNewId(index)).getOrElse(false) => CheckYourAnswersDetailsController.onPageLoad(journeyMode(mode), index, srn)
