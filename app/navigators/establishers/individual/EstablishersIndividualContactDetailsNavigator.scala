...
class EstablishersIndividualContactDetailsNavigator @Inject()(val dataCacheConnector: UserAnswersCacheConnector) extends AbstractNavigator {
  ...
  private def checkUpdateModeRoute(mode: CheckUpdateMode.type, ua: UserAnswers, srn: Option[String]): PartialFunction[Identifier, Call] = {
    case EstablisherEmailId(index) if ua.get(IsEstablisherNewId(index)).getOrElse(false) => cyaIndividualContactDetailsPage(mode, index, srn)
    case EstablisherEmailId(_) => anyMoreChangesPage(srn)
    case EstablisherPhoneId(index) if ua.get(IsEstablisherNewId(index)).getOrElse(false) => cyaIndividualContactDetailsPage(mode, index, srn)
