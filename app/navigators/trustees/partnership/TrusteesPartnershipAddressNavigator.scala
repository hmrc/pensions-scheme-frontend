...
class TrusteesPartnershipAddressNavigator @Inject()(val dataCacheConnector: UserAnswersCacheConnector)
  ...
  private def normalAndCheckModeRoutes(mode: SubscriptionMode,
    ...
    case PartnershipAddressListId(index) if mode == NormalMode =>
      PartnershipAddressYearsController.onPageLoad(mode, index, EmptyOptionalSchemeReferenceNumber)
    ...
    case PartnershipAddressYearsId(index) => trusteeAddressYearsRoutes(mode, ua, index, EmptyOptionalSchemeReferenceNumber)
    case id@PartnershipHasBeenTradingId(index) =>
      booleanNav(id, ua, previousAddressLookup(mode, index, EmptyOptionalSchemeReferenceNumber), cyaAddress(journeyMode(mode), index, EmptyOptionalSchemeReferenceNumber))
    case PartnershipPreviousAddressPostcodeLookupId(index) =>
      PartnershipPreviousAddressListController.onPageLoad(mode, index, EmptyOptionalSchemeReferenceNumber)
...
object TrusteesPartnershipAddressNavigator {
  ...
  private def trusteeAddressYearsRoutes(mode: Mode, ua: UserAnswers, index: Int, srn: OptionalSchemeReferenceNumber): Call =
  ...
  private def trusteeAddressRoute(ua: UserAnswers, mode: Mode, index: Int, srn: OptionalSchemeReferenceNumber): Call = {
