...
@Singleton
class EstablishersCompanyDirectorNavigator @Inject()(val dataCacheConnector: UserAnswersCacheConnector) extends
  AbstractNavigator {
  ...
  override protected def updateRouteMap(from: NavigateFrom, srn: Option[String]): Option[NavigateTo] = normalRoutes
  ...
  protected def normalRoutes(from: NavigateFrom, mode: Mode, srn: Option[String]): Option[NavigateTo] =
    from.id match {
      ...
      case DirectorEnterUTRId(establisherIndex, directorIndex) =>
        NavigateTo.dontSave(routes.DirectorAddressPostcodeLookupController.onPageLoad(mode, establisherIndex,
          directorIndex, srn))
      ...
      case DirectorAddressListId(establisherIndex, directorIndex) =>
        NavigateTo.dontSave(routes.DirectorAddressYearsController.onPageLoad(mode, establisherIndex, directorIndex,
          srn))
  ...
  private def addressYearsRoutes(establisherIndex: Int, directorIndex: Int, mode: Mode, srn: Option[String])(
    ...
    answers.get(DirectorAddressYearsId(establisherIndex, directorIndex)) match {
      case Some(AddressYears.UnderAYear) =>
        NavigateTo.dontSave(routes.DirectorPreviousAddressPostcodeLookupController.onPageLoad(mode, establisherIndex,
          directorIndex, srn))
  ...
  private def addressRoutes(answers: UserAnswers, mode: Mode, srn: Option[String], establisherIndex: Int,
                            directorIndex: Int) = {
    ...
    else if (!answers.get(director.IsNewDirectorId(establisherIndex, directorIndex)).contains(true) && mode ==
      CheckUpdateMode) {
      NavigateTo.dontSave(routes.DirectorConfirmPreviousAddressController.onPageLoad(establisherIndex, directorIndex,
        srn))
    }
    else {
  ...
  protected def commonRoutes(from: NavigateFrom, mode: Mode, srn: Option[String]): Option[NavigateTo] =
    from.id match {
      ...
      case DirectorPreviousAddressPostcodeLookupId(establisherIndex, directorIndex) =>
        NavigateTo.dontSave(routes.DirectorPreviousAddressListController.onPageLoad(mode, establisherIndex,
          directorIndex, srn))
      case ConfirmDeleteDirectorId(establisherIndex) =>
        mode match {
          case CheckMode | NormalMode =>
            NavigateTo.dontSave(
              controllers.register.establishers.company.routes.AddCompanyDirectorsController.onPageLoad(mode, srn,
                establisherIndex))
          case _ =>
            anyMoreChanges(srn)
        }
  ...
  private def addressYearsEditRoutes(establisherIndex: Int, directorIndex: Int, mode: Mode, srn: Option[String])(
    ...
    (
      ...
      case (Some(AddressYears.UnderAYear), CheckUpdateMode, Some(_)) =>
        NavigateTo.dontSave(routes.DirectorConfirmPreviousAddressController.onPageLoad(establisherIndex,
          directorIndex, srn))
  ...
  private def checkYourAnswers(establisherIndex: Int, directorIndex: Int, mode: Mode, srn: Option[String])
  : Option[NavigateTo] =
